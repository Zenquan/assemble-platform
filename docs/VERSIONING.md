...
>
> 进度注记：**0.2.0 出口两门禁均已闭合**；**0.3.0 已发布 v0.3.0**（S1 分态渲染、S2 装配动画、S3 手动拖拽、S4 BOM 树+节拍 HUD）。当前进入 **0.4.x（M3 平台化）**：所选流水线由 assembly-svc 返回 BOM，Babylon 按 BOM 从 model-svc 加载真实 GLB，BOM 树按后端 `stationId` 联动。详见「0.2.x → 0.3.x」切片表与 `ARCHITECTURE.md` §5.1。
...
| **S4** | BOM 树 + 节拍面板（HUD） | ✅ done（v0.3.0 S4；0.4.x 改为后端归属） | `engine/bomtree.ts` 按 `AssemblyStep.stationId` 归组 + `deriveBomTreeState` 状态标注 + `engine/taktpanel.ts` `deriveTaktPanel`/`recommendTargetPerHour` + `api/takt.ts` `fetchTaktSimulation` POST `/takt/simulate` + `components/{BomTreePanel,TaktPanel}.vue` + WorkbenchView 接线 | BOM 树随所选流水线后端 BOM 动态分组、当前 step 在所属工位高亮、done ✓ / pending · / 基座徽 + 点选视口联动；节拍数据真接 takt-svc |
