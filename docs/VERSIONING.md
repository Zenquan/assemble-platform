...
> 当前仓库版本基线：根 `package.json` 记为 `0.3.0`，对应下方 `0.3.x` 行（`v0.3.0` tag 已发）。版本号统一由**根包**号代表「平台整体版本」，各 workspace 包随发布同步主版本（早期用同一主版号更易管理）。
>
> 进度注记：**0.2.0 出口两门禁均已闭合** —— 「浏览器渲染装配」由 `engine/babylon.ts`（FEAT-20260903-002）达成；「本地一键起前端+服务」由 `scripts/dev.mjs`（`pnpm dev`）达成。**0.3.0 已发布 v0.3.0**（FEAT-20260903-003，4 切片全部落地并 FF-merge 回 main）：S1 分态渲染 ✅、S2 装配动画 ✅、S3 手动拖拽 ✅、S4 BOM 树+节拍 HUD ✅；`feat/0.3.0-s1/s2/s3/s4-*` 分支保留不删。下一里程碑为 **0.4.x（M3 平台化：多产线复用）**。详见「0.2.x → 0.3.x」切片表。
...
| **S4** | BOM 树 + 节拍面板（HUD） | ✅ done（v0.3.0 S4） | `engine/bomtree.ts` `groupStepsByStation` round-robin 归组 + `deriveBomTreeState` 状态标注 + `engine/taktpanel.ts` `deriveTaktPanel`/`recommendTargetPerHour` + `api/takt.ts` `fetchTaktSimulation` POST `/takt/simulate` + `components/{BomTreePanel,TaktPanel}.vue` + WorkbenchView 接线（轮询只读 + `frameToPart` 选件联动）+ `vite.config.ts` `/takt->7104` + `scripts/dev.mjs` 精简模式纳入 takt-svc | BOM 树按工位分组、当前 step 在所属工位高亮、done ✓ / pending · / 基座徽 + 点选视口 frameToPart 联动；节拍数据真接 takt-svc（瓶颈/产能/负荷分级 ok·busy·overload）；`docs/s4-bom-takt{,-assembled}.png` 归档；纯逻辑 17 例（bomtree 10 + taktpanel 7） |
