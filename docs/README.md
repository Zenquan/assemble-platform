...
| `HA.md` | 高可用：服务优雅停机、liveness/readiness 分离、gateway 上游健康池 + failover、副本配置化（`UPSTREAM_TARGETS`）+ 子进程退避重启 | 0.5.x M4 加固「HA 部署编排」方向已落地；扩展副本/编排/自愈时读 |
| `ALERTING.md` | 告警推送：规则引擎、告警状态机（分级/去重/恢复）、通用 webhook 推送（企业微信/钉钉群机器人） | 0.5.x M4 加固「容灾与告警推送」方向；扩展告警规则/通道时读 |
| `DISASTER_RECOVERY.md` | 容灾与备份：部署层 checklist（数据备份/服务自愈/多活/恢复演练），代码层自愈见 `HA.md` | 上线/部署前对照容灾验收清单时读 |
