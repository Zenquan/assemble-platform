...
| 三模式仿真 | 自动装配（约束贴合 Slerp 平滑过渡）、手动拖拽、步骤回放，可撤销、可从头演示；**BOM 装配树随进度逐件联动，点选零件即视口聚焦其包围盒** |
| 实时干涉拦截 | 手动拖拽装配走 `clearance-core` 的 `queryInteractive` 本地零往返判定，命中即高亮并拦截落位 |
| 整线离线预检 | `interference-svc` 读真实 BOM / GLB 包络批量 `runFull()`，产线卡片展示命中数、耗时与质量指标 |
| 干涉处理闭环 | 装配工作台内置**干涉处理**：查看 / 复检整线离线报告 → 命中零件一键定位高亮 → 引导转产线配置中心「自动计算避让」回填工位坐标 → 保存后返回复检，直至归零 |
| 节拍仿真 | `takt-svc` 按工位数据计算目标产能、开动率、负荷与瓶颈；面板柱状图高亮瓶颈工位 |
| 产线运行态 | GLB 声明驱动设备运动节点，物料沿 BOM 工位路径流转并高亮当前工位，支持启动 / 暂停 / 复位 |
| 多产线复用 | 分拣 / 净菜 / 冷链 3 类工艺线共享同一 GLB 资产库与 BOM 排布算法（5 条种子线 · 13 项资产） |
| 产线配置中心 | 可视化维护工艺类型、设备 GLB、工位节拍、占用长度、朝向、坐标与卫生转运；**自动计算避让**工位布局；保存即刷新 BOM / 传送段 / 节拍服务 |
| 模型资产库 | 13 项内置 GLB 资产 + 自定义 GLB 上传（≤100MB，自动加 `custom-` 前缀）与下载；删除前按产线引用做防护 |
| 性能监控 | `/perf` 性能监视页集中呈现**渲染侧**（引擎帧率 / draw call / 活动网格 / 顶点数）与**前端侧**（页面 FPS / JS 堆 / LCP / FID / CLS，连续指标 10s 滚动均值）及 telemetry **上报状态**；SimMonitor 埋点经 gateway `/telemetry` 聚合，`/metrics` 供 Prometheus 拉取 |
