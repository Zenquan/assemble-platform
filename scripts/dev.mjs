#!/usr/bin/env node
/**
 * 一键起 dev 环境（精简：assembly-svc + interference-svc + vite 前端）。
 *
 * 背景：sim-platform 的 vite dev 代理把 `/lines` → 7101、`/interference` → 7102，
 * 若后端未起，浏览器会报 `[vite] http proxy error: ECONNREFUSED 127.0.0.1:7101`。
 * 本脚本一次性拉起这两个后端与前端，按端口占用情况跳过已在跑的进程，退出时统一清理。
 *
 * 特性：
